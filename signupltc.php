...
$result=mysqli_query($conn,"select * from exptable where fname='$fname'")
	...
	if (isset($row['fname']) && $row['fname'] == $fname) {
        echo "<center>";
        echo "<div class='card text-white bg-danger mb-3' style='max-width: 18rem;'><div class='card-header'>Account can't be created!</div><div class='card-body'><h5 class='card-title'>Username," . $fname . "</h5><p class='card-text'>Already exists. Please choose another username to create a new account</p><button class='btn-secondary'><a href='home.php'>Go Back</button></div></div>";
        echo "</center>";
    }
